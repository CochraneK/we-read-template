...
HTML = r'''
  ...
  <article class="card wide" id="shelf-explorer"><div class="title"><div><div class="section-kicker">498-book explorer</div><h2>完整书架浏览器</h2></div><small>书名 / 作者 / 类别 / 进度 / 笔记 / 最近阅读；含 secret 书</small></div><div class="explorer-tools"><input id="shelfQuery" type="search" placeholder="搜索书名、作者或类别…"><select id="shelfCategory"><option value="">全部类别</option></select><select id="shelfProgress"><option value="">全部进度</option><option value="noted">有笔记</option><option value="reading">阅读中</option><option value="finished">已读完</option><option value="unknown">进度未知</option></select><select id="shelfSort"><option value="recent">最近阅读</option><option value="notes">笔记最多</option><option value="progress">进度最高</option><option value="title">书名</option></select></div><div class="shelf-summary"><span id="shelfCount"></span><span>只发布书目元数据，不发布原始笔记正文</span></div><div class="shelf-grid" id="shelfGrid"></div><button class="load-more" id="shelfMore" type="button">显示更多</button></article>
'''
